...
train_data = DataLoader(prepare_multi_sources_data.SSLR_Dataset(train_data_path), batch_size=256,
                        shuffle=True, num_workers=0)  # train_data is a tuple: (batch_x, batch_y)
test_data = DataLoader(prepare_multi_sources_data.SSLR_Dataset(test_data_path), batch_size=256,
...
def main():
    ...
    for (batch_x, batch_y, batch_z) in train_data:
        ...
        a_1, _ = model.forward(inputs)
        loss = criterion(a_1, labels)
        # Backward
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
